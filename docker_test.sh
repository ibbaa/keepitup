...
docker run $DOCKER_OPTS --privileged --name build -it --rm -v $PWD:/var/keepitup -w /var/keepitup ibbaa/keepitup:build /bin/bash -c "sudo ./start_emulator.sh && ./gradlew connectedAndroidTest"
RET=$?
