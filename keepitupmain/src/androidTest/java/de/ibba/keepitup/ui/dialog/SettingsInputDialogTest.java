...
import de.ibba.keepitup.test.mock.TestValidator;
...
public class SettingsInputDialogTest extends BaseUITest {
    ...
    public void testValidation() {
        SettingsInputDialog inputDialog = new SettingsInputDialog();
        SettingsInput input = new SettingsInput(SettingsInput.Type.ADDRESS, "abc", "field", Arrays.asList(TestValidator.class.getName(), TestValidator.class.getName()));
        inputDialog.setArguments(input.toBundle());
        inputDialog.show(activity.getSupportFragmentManager(), SettingsInputDialog.class.getName());
        onView(withId(R.id.imageview_dialog_settings_input_ok)).perform(click());
        ...
        onView(allOf(withText("testfailed"), withGridLayoutPosition(1, 1))).check(matches(isDisplayed()));
        onView(allOf(withText("field"), withGridLayoutPosition(2, 0))).check(matches(isDisplayed()));
        onView(allOf(withText("testfailed"), withGridLayoutPosition(2, 1))).check(matches(isDisplayed()));
    ...
    public void testTextColor() {
        SettingsInputDialog inputDialog = new SettingsInputDialog();
        SettingsInput input = new SettingsInput(SettingsInput.Type.ADDRESS, "success", "field", Arrays.asList(TestValidator.class.getName(), TestValidator.class.getName()));
        inputDialog.setArguments(input.toBundle());
        inputDialog.show(activity.getSupportFragmentManager(), SettingsInputDialog.class.getName());
